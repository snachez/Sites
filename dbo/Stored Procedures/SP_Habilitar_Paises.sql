...
BEGIN
	...
	DECLARE @ERROR_NUMBER_SP_Paises NVARCHAR(MAX) = NULL
	DECLARE @ID_ERROR_Paises NVARCHAR(MAX) = CONVERT(INT, ISNULL(SCOPE_IDENTITY(), -1))
	DECLARE @ROW_ERROR_Paises NVARCHAR(MAX) = NULL
	...
	BEGIN
		...
		BEGIN CATCH
			...
			(
			    ...
			    @ERROR_NUMBER_SP_Paises AS ERROR_NUMBER_SP,
			    @ID_ERROR_Paises AS ID,
			    @ROW_ERROR_Paises AS ROW
				FOR JSON PATH, INCLUDE_NULL_VALUES
			)
