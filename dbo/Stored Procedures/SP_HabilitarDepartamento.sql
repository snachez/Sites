...
CREATE PROCEDURE [dbo].[SP_HabilitarDepartamento](
...
BEGIN
	...
	DECLARE @ERROR_NUMBER_SP_Departamento NVARCHAR(MAX) = NULL
	DECLARE @ID_ERROR_Departamento NVARCHAR(MAX) = CONVERT(INT, ISNULL(SCOPE_IDENTITY(), -1))
	DECLARE @ROW_ERROR_Departamento NVARCHAR(MAX) = NULL
	...
	BEGIN
		BEGIN TRY
			...
			BEGIN
				...
				(
					...
					END) AS SUCCESS,
					(CASE 
						WHEN @ROW_Asociados = 0 THEN 'Estados de departamentos actualizados con exito'
						WHEN @ROW_Asociados >= 1 THEN 'No puede desactivar un departamento con áreas o usuarios asociados en estado activo'
					END) AS ERROR_MESSAGE_SP,
		...
		BEGIN CATCH
			...
			(
			    ...
			    @ERROR_NUMBER_SP_Departamento AS ERROR_NUMBER_SP,
			    @ID_ERROR_Departamento AS ID,
			    @ROW_ERROR_Departamento AS ROW
				FOR JSON PATH, INCLUDE_NULL_VALUES
			)
